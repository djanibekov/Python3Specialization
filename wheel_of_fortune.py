...
class WOFHumanPlayer(WOFPlayer):
    ...
    def gerMove(self, category, obscuredPhrase, guessed):
        prompt = """
...
class WOFComputerPlayer(WOFPlayer):
    ...
    def gerPossibleLetters(self, guessed):
        letters_left = [letter for letter in LETTERS if letter not in guessed]
        
        if self.prizeMoney < VOWEL_COST:
            letters_left = [letter for letter in letters_left if letter not in VOWELS]
              
        return letters_left

    def getMove(self, category, obscuredPhrase, guessed): 
        letters = self.gerPossibleLetters(guessed)
        if letters == []:
            return 'pass'
        else:
